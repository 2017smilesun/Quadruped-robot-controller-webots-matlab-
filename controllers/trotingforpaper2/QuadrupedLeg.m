...
%         end
        ...
        %%导数正向运动学
        function vEe = calcVEe(obj,q,qd)  %%已知关节角度q和角速度qd时，求足端速度向量，书公式5.42
            vEe = obj.calcJaco(q) * qd;
        end
...
%             b2y,b3z,b4z,a,b,c;%
        ...
        function inVEe = calcQdd(obj, pEe,vEe,frame)
            q = obj.calcQ(pEe,frame);
            inJaco = inv(obj.calcJaco(q));
            inVEe = inJaco * vEe;
        end
        
        %%逆动力学
        function Tau = calcTau(obj,q,force)
