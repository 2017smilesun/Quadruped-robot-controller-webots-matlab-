classdef LowLevelState < handle
    ...
    methods
        ...
        function  feetvel_cur = getQd(obj)
            ...
            for i = 1:4
                ...
                %fprintf('feetvel_cur的大小: %f %f %f\n',feetvel_cur(1,i),feetvel_cur(2,i),feetvel_cur(3,i));
            end
        end
        ...
        function Gyro_vel = getGyro(obj)  %通过陀螺仪获得绕x/y/z轴的角速度
        ...
        function BodyPos = getBodyPos(obj) %获取机身几何中心在世界坐标系下的坐标
            BodyPos = wb_supervisor_node_get_position(obj.trunk_body)' - [0.536276;-0.130685;0.018512];
        ...
        function GyroGlobal = getGyroGlobal(obj)   %计算
        ...
        function FeetPos = getFeetPos(obj) %获取足端在世界坐标系下的位置坐标
            FeetPos = zeros(3,4);
            FR_foot_pos = wb_supervisor_node_get_position(obj.foot_DEF(1))' - [0.536276;-0.130685;0.018512];
            FL_foot_pos = wb_supervisor_node_get_position(obj.foot_DEF(2))' - [0.536276;-0.130685;0.018512];
            RR_foot_pos = wb_supervisor_node_get_position(obj.foot_DEF(3))' - [0.536276;-0.130685;0.018512];
            RL_foot_pos = wb_supervisor_node_get_position(obj.foot_DEF(4))' - [0.536276;-0.130685;0.018512];
            
            FeetPos(:,1) = FR_foot_pos;
            FeetPos(:,2) = FL_foot_pos;
            FeetPos(:,3) = RR_foot_pos;
            FeetPos(:,4) = RL_foot_pos;
            fprintf('FeetPos的大小: %f %f %f\n',FeetPos(1,1),FeetPos(2,1),FeetPos(3,1));
        end
        
        %function FeetVel = getFeetVel(obj)
        %    FeetVel = obj.Robot.getFeet2BVelocities('GLOBAL');
            %fprintf('FeetVel的大小: %f %f %f\n',FeetVel(1,1),FeetVel(2,1),FeetVel(3,1)); 
         %   for i = 1:4
         %       FeetVel(:,i) = FeetVel(:,i) + obj.getBodyVel();
         %   end
            %fprintf('FeetVel的大小: %f %f %f\n',FeetVel(1,1),FeetVel(2,1),FeetVel(3,1)); 
       % end
